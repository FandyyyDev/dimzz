...
// Start
module.exports = aozoraa = async (aozoraa, m, chatUpdate, store) => {
    try {
        ...
        const botNumber = aozoraa.user.id ? aozoraa.user.id.split(":")[0]+"@s.whatsapp.net" : aozoraa.user.id
        ...
        const groupMetadata = m.isGroup ? await aozoraa.groupMetadata(m.chat).catch(e => {}) : ''
...
if (budy.includes('https://chat.whatsapp.com/')) {
               ...
               let sianj = m.sender
               await aozoraa.groupParticipantsUpdate(m.chat, [sianj], 'remove').then((res) => m.reply(jsonformat(res))).catch((err) => m.reply(jsonformat(err)))
               }
	  }
        // respon
     ...
     * @returns
     */
    aozoraa.send5Vid = async (jid , text = '' , footer = '', vid, but = [], options = {}) =>{
        let message = await prepareWAMessageMedia({ video: vid }, { upload: aozoraa.waUploadToServer })
        ...
        videoMessage: message.videoMessage,
            ...
            aozoraa.relayMessage(jid, template.message, { messageId: template.key.id })
    }
        // TextTeplate 
        const textTemplateButtons = (from, text, footer, buttons) => {
            return aozoraa.sendMessage(from, { text: text, footer: footer, templateButtons: buttons })
        }
       // Detect User Banned
        if (ban[m.sender] && isCmd) {
            await aozoraa.sendText(m.chat, `Maaf @${m.sender.split("@")[0]} Anda Telah Dibanned, Chat Owner Untuk Un Banned`, m, { contextInfo: { mentionedJid: [m.sender] }})
            return
        }
...
user.afkReason = ''
        ...
        aozoraa.sendReadReceipt(m.chat, m.sender, [m.key.id])
        aozoraa.sendPresenceUpdate('composing', m.chat)
        if (isCmd && !isGroup) {
...
console.log(err)
        ...
        let messages = await generateWAMessage(m.chat, { text: text, mentions: mentionedJid }, {
            userJid: aozoraa.user.id,
            quoted: m.quoted && m.quoted.fakeObj
        })
        messages.key.fromMe = areJidsSameUser(m.sender, aozoraa.user.id)
        ...
        aozoraa.ev.emit('messages.upsert', msg)
        }
...
${isSurender ? '' : ``}`.trim()
	    aozoraa.sendText(m.chat, caption, m, { contextInfo: { mentionedJid: parseMention(caption) }}).then(mes => { return _family100['family100'+m.chat].pesan = mesg }).catch(_ => _)
...
${m.pushName} Telah Afk${text ? ': ' + text : ''}`)
        ...
        case 'kuismath': case 'math': {
                ...
                aozoraa.sendText(m.chat, `*Berapa hasil dari: ${result.soal.toLowerCase()}*?\n\nWaktu: ${(result.waktu / 1000).toFixed(2)} detik`, m).then(() => {
		...
		let random = anu[Math.floor(Math.random() * anu.length)]
		    ...
		    id: 'family100'+m.chat,
		     pesan: await aozoraa.sendText(m.chat, hasil, m),
		    ...random,
            ...
            case 'tebak': {
                if (!text) throw `Option:\n1. kata\n2. bendera\n3. kalimat\n4. Siapa\n5. tebakan\n6. lirik\n7. kimia\n\nExample: tebak kalimat`
                  ...
                  if (args[0] === 'kata') {
                      ...
                      aozoraa.sendText(m.chat, `Silahkan Jawab Pertanyaan Berikut\n\n${result.soal}\nWaktu : 60s`, m).then(() => {
                      ...
                      let result = anu[Math.floor(Math.random() * anu.length)]
                      aozoraa.sendFile(m.chat, result.img, '', `Silahkan Jawab Pertanyaan Diatas\n\nCode : ${result.flag}\nWaktu : 60s`, m).then(() => {
		      ...
		      if (tebakkabupaten.hasOwnProperty(m.sender.split('@')[0])) throw "Masih Ada Sesi Yang Belum Diselesaikan!"
                      ...
                      let result = anu[Math.floor(Math.random() * anu.length)]
                      aozoraa.sendFile(m.chat, result.url, '', `Gambar Diatas Adalah Gambar dari Kabupaten?\nWaktu : 60s`, m).then(() => {
		      ...
		      if (tebakkimia.hasOwnProperty(m.sender.split('@')[0])) throw "Masih Ada Sesi Yang Belum Diselesaikan!"
                      ...
                      let result = anu[Math.floor(Math.random() * anu.length)]
                      aozoraa.sendText(m.chat, `Apa Arti Dari Simbol : *${result.lambang}*?\nWaktu : 60s`, m).then(() => {
		      ...
		      if (tebaklirik.hasOwnProperty(m.sender.split('@')[0])) throw "Masih Ada Sesi Yang Belum Diselesaikan!"
                      ...
                      let result = anu[Math.floor(Math.random() * anu.length)]
                      aozoraa.sendText(m.chat, `Ini Adalah Lirik Dari Lagu? : *${result.soal}*?\nWaktu : 60s`, m).then(() => {
...
var dash = `DASHBOARD N E Z U K O
${tg}`
m.reply(dash)
}
break
            case 'caklontong': {
                ...
                let result = anu[Math.floor(Math.random() * anu.length)]
                aozoraa.sendText(m.chat, `*Jawablah Pertanyaan Berikut :*\n${result.soal}*\nWaktu : 60s`, m).then(() => {
            ...
            case 'candy': case 'christmas': case '3dchristmas': case 'sparklechristmas': case 'deepsea': case 'scifi': case 'rainbow': case 'waterpipe': case 'spooky': case 'pencil': case 'circuit': case 'discovery': case 'metalic': case 'fiction': case 'demon': case 'transformer': case 'berry': case 'thunder': case 'magma': case '3dstone': case 'neonlight': case 'glitch': case 'harrypotter': case 'brokenglass': case 'papercut': case 'watercolor': case 'multicolor': case 'neondevil': case 'underwater': case 'graffitibike': case 'snow': case 'cloud': case 'honey': case 'ice': case 'fruitjuice': case 'biscuit': case 'wood': case 'chocolate': case 'strawberry': case 'matrix': case 'blood': case 'dropwater': case 'toxic': case 'lava': case 'rock': case 'bloodglas': case 'hallowen': case 'darkgold': case 'joker': case 'wicker': case 'firework': case 'skeleton': case 'blackpink': case 'sand': case 'glue': case '1917': case 'leaves': {
             	if (!text) throw `Example : ${prefix + command} aozoraa`
             	m.reply(`SABAR YAAAA....`)
             	let link
                ...
                aozoraa.sendMessage(m.chat, { image: { url: anu }, caption: 'Textpro Scraper By aozoraaxd' }, { quoted: m })
             }
             break
case 'tes':
aozoraa.sendText(`${sender}`, `Tester`, m)
break
case 'otp':
...
case 'ppuser': {
                    ...
                    try {
                        ppuser = await aozoraa.profilePictureUrl(who, 'image')
                    } catch {
                        ppuser = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
                    }
                    let about = (await aozoraa.fetchStatus(sender).catch(console.error) || {}).status || ''
                    let str = `
⭔ Name : ${pushname}
⭔ Link : https://wa.me/${sender.split`@`[0]}
⭔ Registered : ${isUser ? 'Yes (' + new Date(regTime) + ')': 'No'}
`.trim()
                    aozoraa.sendMessage(m.chat, { image: { url: ppuser }, caption: str }, { quoted: m })
                }
                break
             ...
             case '3d': case 'window': case 'signature': case 'galaxynimw': case 'avatar': case 'onepiece': case 'boom': case 'anonymous': {// 1 text with radio
                    ...
                    if (args.length == 0) return m.reply(`Example: ${prefix + command} aozoraaxd`)
                    m.reply(`SABAR YAAAA....`)
					let link
					let radioo
             ...
             aozoraa.sendMessage(m.chat, { image: { url: 'https://e1.yotools.net'+`${anu.image}` }, caption: 'Ephoto Scraper By aozoraaxd' }, { quoted: m })
            }
            break
				case 'mascot': case 'glitter': { // 2 text with radio
				
                    if (args.length == 0) return m.reply(`Example: ${prefix + command} aozoraa|bot`) 
                    m.reply(`SABAR YAAAA....`)
					let link
					let radioo
					...
					aozoraa.sendMessage(m.chat, { image: { url: 'https://e1.yotools.net'+`${anu.image}` }, caption: 'Textpro Scraper By aozoraaxd' }, { quoted: m })
				}
				break
				case 'valorant': { // 3 text with radio
                    if (args.length == 0) return m.reply(`Example: ${prefix + command} aozoraa|bot|aozoraaxd`)
                    m.reply(`SABAR YAAAA....`)
					let link
					let radioo
				...
				case 'ballon':
             	...
             	m.reply(`SABAR YAAAA....`)
					...
					aozoraa.sendImage(m.chat, anu, 'nih jadi', m)
					break
					case 'pantaii':
				 photofunia = require('./lib/photofunia.js')
					if (!text) throw `Example : ${prefix + command} aozoraa`
...
aozoraa.sendMessage(m.chat, { image: { url: a[0].thumbnail }, caption: `${result}` }, { quoted: m})
break
             break
...
aozoraa.sendMessage(m.chat, { video: { url: huh }, caption: `NIH BANG GAUSH TRIMAKASIH` }, { quoted: m})
...
case 'fetch': case 'get': {
			...
			if (!/text|json/.test(res.headers.get('content-type'))) return aozoraa.sendFile(m.chat, isUrl(text)[0], 'file', '', m)
		...
		break
             ...
             aozoraa.sendMessage(m.chat, { image: { url: random }, caption: `© AOZORA BOTZ` }, { quoted: m})
             break
             case 'hinata':
             m.reply(`SABAR YAAAA....`)
...
aozoraa.sendMessage(m.chat, { image: anu, caption: `© MAGER-NULIS BY NEZUKO` }, { quoted: m})
break
	    ...
	    case 'family100': {
            ...
            case 'tebak': {
                ...
                if (args[0] === "lagu") {
                    ...
                    let anu = await fetchJson('https://aozoraaarridho.my.id/database/games/tebaklagu.json')
                    let result = anu[Math.floor(Math.random() * anu.length)]
                    let msg = await aozoraa.sendMessage(m.chat, { audio: { url: result.link_song }, mimetype: 'audio/mpeg' }, { quoted: m })
                    aozoraa.sendText(m.chat, `Lagu Tersebut Adalah Lagu dari?\n\nArtist : ${result.artist}\nWaktu : 60s`, msg).then(() => {
                    tebaklagu[m.sender.split('@')[0]] = result.jawaban.toLowerCase()
                    })
                    await sleep(60000)
                    if (tebaklagu.hasOwnProperty(m.sender.split('@')[0])) {
                    console.log("Jawaban: " + result.jawaban)
                    aozoraa.send1ButMes(m.chat, `Waktu Habis\nJawaban: ${result.jawaban}\n\nIngin bermain lagi? tekan button dibawah`, `© ${ownername}`, `${prefix}tebak lagu`, `Tebak Lagu`, m)
                    ...
                    let result = anu[Math.floor(Math.random() * anu.length)]
                    aozoraa.sendImage(m.chat, result.img, `Silahkan Jawab Soal Di Atas Ini\n\nDeskripsi : ${result.deskripsi}\nWaktu : 60s`, m).then(() => {
...
m.reply(`SABAR YAAAA....`)
let media = await aozoraa.downloadAndSaveMediaMessage(quoted)
let ran = getRandom('.mp3')
exec(`ffmpeg -i ${media} ${set} ${ran}`, (err, stderr, stdout) => {
fs.unlinkSync(media)
if (err) return m.reply(err)
let buff = fs.readFileSync(ran)
aozoraa.sendMessage(m.chat, {audio: buff, ptt: true, mimetype: 'audio/mpeg'}, {quoted:m})
fs.unlinkSync(ran)
})
...
case 'ban': case 'banned': {
		...
		break
            ...
            let ments = [me, jodoh]
            aozoraa.sendTextWithMentions(m.chat, jawab, m)
            }
            break
            case 'jadian': {
            if (!m.isGroup) throw  m.reply(`Khusus Group`)
            ...
            case 'join': {
                ...
                await aozoraa.groupAcceptInvite(result).then((res) => m.reply(jsonformat(res))).catch((err) => m.reply(jsonformat(err)))
            }
            break
            case 'leave': {
                if (!isCreator) throw  m.reply(`Lu Siapa Ajg`)
                await aozoraa.groupLeave(m.chat).then((res) => m.reply(jsonformat(res))).catch((err) => m.reply(jsonformat(err)))
            }
            break
	case 'kick': case 'tendang': {
		...
		await aozoraa.groupParticipantsUpdate(m.chat, [users], 'remove').then((res) => m.reply(jsonformat(res))).catch((err) => m.reply(jsonformat(err)))
	}
	break
case 'antilink':
    if (!m.isGroup) throw  m.reply(`Khusus Group`)
    ...
    if (!isGroupAdmins) throw  m.reply(`Khusus Admin`)
					if (args[0] === 'on') {
						...
						aozoraa.sendMessage(m.chat,  {text: `ALLERT!!! Group ini sudah di pasang anti link\nJika Kamu Melanggar Maka Akan Saya Tendang`})
					} else if (args[0] === 'off') {
 ...
 m.reply(`Pilih Antilink On / Off `)
	...
	case 'add': {
		...
		await aozoraa.groupParticipantsUpdate(m.chat, [users], 'add').then((res) => m.reply(jsonformat(res))).catch((err) => m.reply(jsonformat(err)))
	}
	break
	case 'promote': {
		if (!m.isGroup) throw  m.reply(`Khusus Group`)
                ...
                if (!isGroupAdmins) throw  m.reply(`Khusus Admin`)
		let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
		await aozoraa.groupParticipantsUpdate(m.chat, [users], 'promote').then((res) => m.reply(jsonformat(res))).catch((err) => m.reply(jsonformat(err)))
	}
	break
	case 'demote': {
		if (!m.isGroup) throw  m.reply(`Khusus Group`)
                ...
                if (!isGroupAdmins) throw  m.reply(`Khusus Admin`)
		let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
		await aozoraa.groupParticipantsUpdate(m.chat, [users], 'demote').then((res) => m.reply(jsonformat(res))).catch((err) => m.reply(jsonformat(err)))
	}
	break
    case 'tagall': {
        if (!m.isGroup) throw  m.reply(`Khusus Group`)
...
let teks = `Tag all\n`
        ...
        aozoraa.sendMessage(m.chat, { text: teks, mentions: participants.map(a => a.id) }, { quoted: m })
        }
        break
    case 'taginfo': {
...
m.reply(`SABAR YAAAA....`)
mee = await aozoraa.downloadAndSaveMediaMessage(quoted)
mem = await TelegraPh(mee)
meme = `https://api.memegen.link/images/custom/-/${text}.png?background=${mem}`
memek = await aozoraa.sendImageAsSticker(m.chat, meme, m, { packname: global.packname, author: global.author })
...
aozoraa.sendMedia(m.chat, ok, '', m, { caption: 'ok' })
break

                case 'hidetag': {
if (!isCreator) throw  m.reply(`Lu Siapa Ajg`)

            aozoraa.sendMessage(m.chat, { text : q ? q : '' , mentions: participants.map(a => a.id)}, { quoted: m })
            }
            break
	    case 'setname': case 'setsubject': {
                ...
                if (!isGroupAdmins) throw  m.reply(`Khusus Admin`)
                if (!text) throw 'Text ?'
                await aozoraa.groupUpdateSubject(m.chat, text).then((res) => m.reply(jsonformat(res))).catch((err) => m.reply(jsonformat(err)))
            }
            break
            ...
            case 'group': case 'grup': {
                ...
                if (!isGroupAdmins) throw  m.reply(`Khusus Admin`)
                if (args[0] === 'close') {
                    await aozoraa.groupSettingUpdate(m.chat, 'announcement').then((res) => m.reply(`Sukses Menutup Group`)).catch((err) => m.reply(jsonformat(err)))
                } else if (args[0] === 'open') {
                    await aozoraa.groupSettingUpdate(m.chat, 'not_announcement').then((res) => m.reply(`Sukses Membuka Group`)).catch((err) => m.reply(jsonformat(err)))
                } else {
                m.reply('OPEN? CLOSE?')
            }
            }
            ...
            case 'linkgroup': case 'linkgc': {
                if (!m.isGroup) throw  m.reply(`Khusus Group`)
                let response = await aozoraa.groupInviteCode(m.chat)
                aozoraa.sendText(m.chat, `https://chat.whatsapp.com/${response}\n\nLink Group : ${groupMetadata.subject}`, m, { detectLink: true })
            }
            break
            ...
            case 'delete': case 'del': {
                ...
                aozoraa.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: true, id: m.quoted.id, participant: m.quoted.sender } })
            }
            break
            ...
            case 'bcimg': case 'bcvidio': case 'bcaudio': {
                ...
                let getGroups = await aozoraa.groupFetchAllParticipating()
                ...
                for (let i of anu) {
                    ...
                    if (/webp/.test(mime)) {
                    aozoraa.sendMessage(i, { sticker: { url: media } }, { quoted: fkontak })
                    } else if (/image/.test(mime)) {
                    let junn = `${text}\n\n-PESAN SIARAN`
                    aozoraa.send5ButImg(i, junn, `nezuko`, buffer, butoon)
                    } else if (/video/.test(mime)) {
                    let junn = `${text}\n\n-PESAN SIARAN`
                    aozoraa.send5Vid(i, junn, `nezuko`, buffer, butoon)
                    } else if (/audio/.test(mime)) {
                    aozoraa.sendMessage(i, {audio: buffer, mimetype: 'audio/mpeg'}, { quoted : m })
                    } else {
		    ...
		    let btn = [{
                quickReplyButton: {
                    ...
                    id: 'owner'
                        ...
                        let txt = `${text}\n\n-PESAN SIARAN`
                      textTemplateButtons(yoi, txt, `Broadcast Bot Nezuko`, btn)
		}
		...
		console.log(chalk.black(chalk.bgWhite('[ Succes ]')))
            ...
            case 'infochat': {
                ...
                aozoraa.sendTextWithMentions(m.chat, teks, m)
            }
            break
            case 'q': case 'quoted': {
		if (!m.quoted) return m.reply('Reply Pesannya!!')
		let wokwol = await aozoraa.serializeM(await m.getQuotedObj())
            ...
            case 'listpc': {
             ...
             break
                case 'listgc': {
                 ...
                 for (let i of anu) {
                     let metadata = await aozoraa.groupMetadata(i)
             ...
             case 'listonline': case 'liston': {
                    ...
                    let online = [...Object.keys(store.presences[id]), aozoraa.user.id]
                    aozoraa.sendText(m.chat, 'List Online:\n\n' + online.map(v => '│➻ @' + v.replace(/@.+/, '')).join`\n`, m, { mentions: online })
             }
             break
		...
		if (!quoted) throw `Balas Video/Image Dengan Caption ${prefix + command}`
		    ...
		    let encmedia = await aozoraa.sendImageAsSticker(m.chat, media, m, { packname: global.packname, author: global.author })
		    await fs.unlinkSync(encmedia)
		} else if (/video/.test(mime)) {
		    if ((quoted.msg || quoted).seconds > 11) return m.reply('Maksimal 10 detik!')
		    let media = await quoted.download()
		    let encmedia = await aozoraa.sendVideoAsSticker(m.chat, media, m, { packname: global.packname, author: global.author })
		    await fs.unlinkSync(encmedia)
                } else if (quoted.mentionedJid) {
                    let media = await aozoraa.profilePictureUrl(quoted.mentionedJid[0], 'image')
                    let encmedia = await aozoraa.sendMediaAsSticker(m.chat, media, m, { packname: global.packname, author: global.author })
            	    await fs.unlinkSync(encmedia)
		} else if (isUrl(args[0])) {
                    if (isUrl(args[0])) encmedia = await aozoraa.sendMediaAsSticker(m.chat, isUrl(text)[0], m, { packname: global.packname, author: global.author })
		    else return m.reply('URL tidak valid!')
		    await fs.unlinkSync(encmedia)
		} else if (text) {
		    let anu = await xfarr.StickerSearch(text)
                    for (let i = 0; i < (anu.sticker_url.length < 5 ? anu.sticker_url.length : 5); i++) {
                    	let encmedia = await aozoraa.sendMediaAsSticker(m.chat, anu.sticker_url[i], m, { packname: packname, author: author })
                    	await fs.unlinkSync(encmedia)
                    }
...
aozoraa.sendMessage(m.chat, { sticker: { url: encmedia } }, { quoted: m })
}
break
            case 'emojimix': {
		...
		for (let res of anu.results) {
		    let encmedia = await aozoraa.sendImageAsSticker(m.chat, res.url, m, { packname: global.packname, author: global.author, categories: res.tags })
		    await fs.unlinkSync(encmedia)
		}
	    }
	    break
            case 'toimage': case 'toimg': {
                ...
                exec(`ffmpeg -i ${media} ${ran}`, (err) => {
                    ...
                    aozoraa.sendMessage(m.chat, { image: buffer }, { quoted: m })
                    fs.unlinkSync(ran)
                })
            }
            break
            ...
            hasilnya = 'Convert Mp4 to Mp3'
                ...
                m.reply(`SABAR YAAAA....`)
               media = await aozoraa.downloadAndSaveMediaMessage(quoted)
               ran = getRandom('.mp3')
               exec(`ffmpeg -i ${media} ${ran}`, (err) => {
               fs.unlinkSync(media)
               if (err) return m.reply(`Err: ${err}`)
               buffer453 = fs.readFileSync(ran)
               aozoraa.sendMessage(m.chat, { document: buffer453 , mimetype: 'audio/mp3', fileName: `${hasilnya}.mp3` }, { quoted: m})
               fs.unlinkSync(ran)
})
            break
		...
		let { webp2mp4File } = require('./lib/uploader')
                ...
                await aozoraa.sendMessage(m.chat, { video: { url: webpToMp4.result, caption: 'Convert Webp To Video' } }, { quoted: m })
                await fs.unlinkSync(media)
            }
            break
            case 'togif': {
		...
		let { webp2mp4File } = require('./lib/uploader')
                let media = await aozoraa.downloadAndSaveMediaMessage(quoted)
                let webpToMp4 = await webp2mp4File(media)
                await aozoraa.sendMessage(m.chat, { video: { url: webpToMp4.result, caption: 'Convert Webp To Video' }, gifPlayback: true }, { quoted: m })
                await fs.unlinkSync(media)
            }
            break
	        case 'tourl': {
	    ...
	    case 'yts': case 'ytsearch': {
                ...
                aozoraa.sendMessage(m.chat, { image: { url: search.all[0].thumbnail },  caption: teks }, { quoted: m })
            }
            break
        case 'google': {
                if (!text) throw `Example : ${prefix + command} aozoraa arridho`
...
🔗 *Media Url* : ${images}`,
                ...
                aozoraa.sendMessage(m.chat, buttonMessage, { quoted: m })
        })
        }
        break
		...
		let anu = await xfarr.Telesticker(isUrl(text)[0])
                for (let i = 0; i < (anu.length < 10 ? anu.length : 10); i++) {
                    let encmedia = await aozoraa.sendImageAsSticker(m.chat, anu[i].url, m, { packname: packname, author: author })
                    await fs.unlinkSync(encmedia)
                }
	    }
	    break
            case 'zippyshare': case 'zippydl': {
                ...
                aozoraa.sendText(m.chat, util.format(anu), m)
                aozoraa.sendMedia(m.chat, anu.link, '', m)
            }
            break
		...
		aozoraa.sendMessage(m.chat, { image: { url: anu.thumbnail }, caption: util.format(anu) }, { quoted: m })
                aozoraa.sendMedia(m.chat, result.url, '', m)
            }
            break
	    case 'stickersearch': case 'ssearch': {
            ...
            break
            case 'tutormp3':
                oaozoraa = `SALIN LINK URL DIATAS LALU KETIK
ytmp3 <link url diatas>`
m.reply(oaozoraa)
break
case 'tutormp4':
                oaozoraa2 = `SALIN LINK URL DIATAS LALU KETIK
...
│➻ Url : ${anu.url}`,
            ...
            case 'ytmp3': {
              ...
              if (Number(anu.size.split("MB")[0]) >= 9000) return m.reply('File Melebihi Batas '+util.format(anu))
                aozoraa.sendImage(m.chat, `${anu.thumb}`, `Title: ${anu.title}\n Channel: ${anu.channel}\n Like: ${anu.likes}\n Dislike: ${anu.dislike}\n Views: ${anu.views}\nQuality: ${anu.quality}\n Size: ${anu.size}\n Upload: ${anu.uploadDate}\nDesc: ${anu.desc}`, m)
                aozoraa.sendMessage(m.chat, { audio: { url: anu.result }, mimetype: 'audio/mp4', fileName: `${anu.title}.mp3\n\nFile akan di kirim mohon sabar` }, { quoted: m })
                }
                break
            case 'ytmp4': {
            if (!text) throw `Example: ytmp4 https://youtu.be/BkBL0Rx-Dtw`
           let urls = quoted.text.match(new RegExp(/(?:https?:\/\/)?(?:youtu\.be\/|(?:www\.|m\.)?youtube\.com\/(?:watch|v|embed|shorts)(?:\.php)?(?:\?.*v=|\/))([a-zA-Z0-9\_-]+)/, 'gi'))
            ...
            anu = await ytMp4(text)
                    if (Number(anu.size.split("MB")[0]) >= 9000) return m.reply('File Melebihi Batas '+util.format(anu))
            aozoraa.sendMessage(m.chat, { video: { url: anu.result }, fileName: `${anu.title}.mp4`, mimetype: 'video/mp4', caption: `Title: ${anu.title}\n Channel: ${anu.channel}\n Like: ${anu.likes}\n Dislike: ${anu.dislike}\n Views: ${anu.views}\nQuality: ${anu.quality}\n Size: ${anu.size}\n Upload: ${anu.uploadDate}\nDesc: ${anu.desc}` }, { quoted: m })
            }
            break
...
`.trim()
                aozoraa.sendText(m.chat, teks, m, { mentions: Object.values(cmdmedia).map(x => x.mentionedJid).reduce((a,b) => [...a, ...b], []) })
            }
            break
            case 'lockcmd': {
                if (!isCreator) throw  m.reply(`Lu Siapa Ajg`)
...
Lihat list Pesan Dengan ${prefix}listmsg`)
            ...
            case 'getmsg': {
                ...
                aozoraa.copyNForward(m.chat, msgs[text.toLowerCase()], true)
            }
            break
            case 'listmsg': {
                let msgs = JSON.parse(fs.readFileSync('./lib/db/msg.json'))
...
break
	    case 'anonymous': {
                ...
                aozoraa.sendButtonText(m.chat, buttons, `\`\`\`Hi ${await aozoraa.getName(m.sender)} Welcome To Anonymous Chat\n\nKlik Button Dibawah Ini Untuk Mencari Partner\`\`\``, aozoraa.user.name, m)
            }
			break
            case 'keluar': case 'leave': {
                ...
                if (!room) {
                    ...
                    await aozoraa.sendButtonText(m.chat, buttons, `\`\`\`Kamu Sedang Tidak Berada Di Sesi Anonymous, Tekan Button Untuk Mencari Partner \`\`\``)
                    throw false
                }
                m.reply('Ok bos')
                let other = room.other(m.sender)
                if (other) await aozoraa.sendText(other, `\`\`\`Partner Telah Meninggalkan Sesi Anonymous\`\`\``, m)
            ...
            case 'sendkontak': case 'sendcontact': {
                ...
                let profile = await aozoraa.profilePictureUrl(room.b)
                let status = await aozoraa.fetchStatus(room.b)
                let msg = await aozoraa.sendImage(room.a, profile, `Nama : ${await aozoraa.getName(room.b)}\nBio : ${status.status}\nUser : @${room.b.split("@")[0]}`, m, { mentions: [room.b] })
                aozoraa.sendContact(room.a, [room.b.split("@")[0]], msg)
            }
            break
            case 'mulai': case 'start': {
                ...
                if (Object.values(this.anonymous).find(room => room.check(m.sender))) {
                    ...
                    await aozoraa.sendButtonText(m.chat, buttons, `\`\`\`Kamu Masih Berada Di dalam Sesi Anonymous, Tekan Button Dibawah Ini Untuk Menghentikan Sesi Anonymous Anda\`\`\``, aozoraa.user.name, m)
                    throw false
                }
                ...
                if (room) {
                    ...
                    await aozoraa.sendButtonText(room.a, buttons, `\`\`\`Berhasil Menemukan Partner, sekarang kamu dapat mengirim pesan\`\`\``, aozoraa.user.name, m)
                    room.b = m.sender
                    room.state = 'CHATTING'
                    await aozoraa.sendButtonText(room.b, buttons, `\`\`\`Berhasil Menemukan Partner, sekarang kamu dapat mengirim pesan\`\`\``, aozoraa.user.name, m)
                } else {
                    let id = + new Date
                    this.anonymous[id] = {
                        id,
                    ...
                    await aozoraa.sendButtonText(m.chat, buttons, `\`\`\`Mohon Tunggu Sedang Mencari Partner\`\`\``, aozoraa.user.name, m)
                }
                break
            }
...
${cpus.map((cpu, i) => `${i + 1}. ${cpu.model.trim()} (${cpu.speed} MHZ)\n${Object.keys(cpu.times).map(type => `- *${(type + '*').padEnd(6)}: ${(100 * cpu.times[type] / cpu.total).toFixed(2)}%`).join('\n')}`).join('\n\n')}` : ''}
            ...
            case 'owner': case 'creator': {
                aozoraa.sendContact(m.chat, global.owner, m)
            }
            break
            case 'runtime':
...
let btn = [{
    urlButton: {
        ...
        url: 'https://chat.whatsapp.com/EQucBehAAMfECDvYJziu71'
    }
},  {
    quickReplyButton: {
        displayText: 'RULES',
...
aozoraa.send5ButImg(m.chat, Anu, ` MENU BOT `, fatihgans, btn)
                                          break
                                          case 'command':{
                                            let template = await generateWAMessageFromContent(m.chat, proto.Message.fromObject({
                                                            listMessage :{
                                                                ...
                                                                footerText: `Nezuko Version : 2.0.1`,
                                                      ...
                                                      listType: 1
                                                        ...
                                                        aozoraa.relayMessage(m.chat, template.message, { messageId: template.key.id })
                                                        }
                                                        break
                                          case 'pulsa':
                                              m.reply(`ANDA MEMILIH DONASI PULSA\nTRANSFER KE 087772051527`)
                                              break
                                          case 'donasi':
                    Sup = `
╭───[ *SUPPORT - B O T*  ]───
...
let bton = [{
    urlButton: {
        ...
        url: 'https://telegra.ph/file/85d49e8aaef5dee95b2a8.jpg'
    }
}, {
    quickReplyButton: {
...
aozoraa.send5ButImg(m.chat, Sup, ` SUPPORT BOT `, donasi, bton)
                                          break
...
│➻ ${prefix}birthday
 ...
 let butong = [{
    urlButton: {
       ...
       url: 'https://instagram.com/dimz0021'
        }
   },{
    quickReplyButton: {
 ...
 aozoraa.send5ButImg(m.chat, okok, ` ALL MENU BOT `, fatihgan, butong)
                                          break
...
│➻ ${prefix}voice <option>
 ...
 aozoraa.send5ButImg(m.chat, okok1, ` Jangan Lupa Donasi `, fatihgan, butong1)
break
 case 'groupmenu':
...
│➻ ${prefix}hidetag
 ...
 aozoraa.send5ButImg(m.chat, okok2, ` Jangan Lupa Donasi `, fatihgan, butong1)
break
 case 'anonymousmenu':
...
│➻ ${prefix}anonymous
 ...
 aozoraa.send5ButImg(m.chat, okok3, ` Jangan Lupa Donasi `, fatihgan, butong1)
break
 case 'toolsmenu':
...
│➻ ${prefix}nulis
 ...
 aozoraa.send5ButImg(m.chat, okok4, ` Jangan Lupa Donasi `, fatihgan, butong1)
break
 case 'searchmenu':
...
│➻ ${prefix}wikimedia <query>
 ...
 aozoraa.send5ButImg(m.chat, okok5, ` Jangan Lupa Donasi `, fatihgan, butong1)
break
  case 'downloadmenu':
...
│➻ ${prefix}mediafire <link>
 ...
 aozoraa.send5ButImg(m.chat, okok6, ` Jangan Lupa Donasi `, fatihgan, butong1)
break
...
│➻ ${prefix}jodohku
 ...
 aozoraa.send5ButImg(m.chat, okok7, ` Jangan Lupa Donasi `, fatihgan, butong1)
break
 case 'gamemenu':
...
│➻ ${prefix}tebak <option>
 ...
 aozoraa.send5ButImg(m.chat, okok8, ` Jangan Lupa Donasi `, fatihgan, butong1)
break
 case 'ownermenu':
...
│➻ ${prefix}> / $ / => / <
 ...
 aozoraa.send5ButImg(m.chat, okok9, ` Jangan Lupa Donasi `, fatihgan, butong1)
break
 case 'othermenu':
...
│➻ ${prefix}owner
 ...
 aozoraa.send5ButImg(m.chat, okok10, ` Jangan Lupa Donasi `, fatihgan, butong1)
break
  case 'databasemenu':
...
│➻ ${prefix}listmsg
 ...
 aozoraa.send5ButImg(m.chat, okok11, ` Jangan Lupa Donasi `, fatihgan, butong1)
break
 case 'animemenu':
...
│➻ ${prefix}nezuko
 ...
 aozoraa.send5ButImg(m.chat, okok12, ` Jangan Lupa Donasi `, fatihgan, butong1)
break
case 'videomenu':
...
│➻ ${prefix}felisa
 ...
 aozoraa.send5ButImg(m.chat, okok09, ` Jangan Lupa Donasi `, fatihgan, butong1)
break
  case 'makermenu':
...
│➻ ${prefix}birthday                 
 ...
 aozoraa.send5ButImg(m.chat, okok13, ` Jangan Lupa Donasi `, fatihgan, butong1)
break          
                                          case 'block': {
                                            if (!isCreator) throw  m.reply(`Lu Siapa Ajg`)
                                            let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
                                            await aozoraa.updateBlockStatus(users, 'block').then((res) => m.reply(jsonformat(res))).catch((err) => m.reply(jsonformat(err)))
                                        }
                                        m.reply(`succes`)
                                        break
                                            case 'unblock': {
                                            if (!isCreator) throw  m.reply(`Lu Siapa Ajg`)
                                            let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
                                            await aozoraa.updateBlockStatus(users, 'unblock').then((res) => m.reply(jsonformat(res))).catch((err) => m.reply(jsonformat(err)))
                                        }
                                        m.reply(`succes`)
                                        break
            case 'sewa':
...
╰─────────`
textTemplateButtons(m.chat, okbor, `Sewa Bot Aozora`, btn1)
                    break
            case 'ig':
                case 'igdl':
ok = await xfarr.Instagram(text)
aozoraa.sendMessage(m.chat, { video: { url: ok.medias[0].url, caption: 'Done' } }, { quoted: m })
break
            case 'tt':
                case 'tiktok':
                    case 'tiktoknowm':
            ...
            if (!text) return m.reply('Linknya?')
             ...
             aozoraa.sendMessage(m.chat, { video: { url: anu.nowm }, caption: `Nih Kak` }, { quoted: m})
             break
             case 'setpp': {
                ...
                let media = await aozoraa.downloadAndSaveMediaMessage(quoted)
                await aozoraa.updateProfilePicture(botNumber, { url: media }).catch((err) => fs.unlinkSync(media))
    ...
    aozoraa.sendText('6281220670449@s.whatsapp.net', `Bug Report\n${text}\n Dari ${pushname}`, m)
...
case 'saran':
    ...
    if (text.length < 10) throw `Laporan terlalu pendek, minimal 10 karakter!`
    if (text.length > 1000) throw `Laporan terlalu panjang, maksimal 1000 karakter!`
    aozoraa.sendText('6281220670449@s.whatsapp.net', `SARAN BANG\n${text}\n Dari ${pushname}`, m)
    m.reply(`_Pesan terkirim kepemilik bot, jika ${command.toLowerCase()} hanya main-main tidak akan ditanggapi._`)
break
            case 'rules':
            case 'tos':
...
m.reply(e)
		...
		if (isCmd && budy.toLowerCase() != undefined) {
		    ...
		    aozoraa.copyNForward(m.chat, msgs[budy.toLowerCase()], true)
		}
        }
